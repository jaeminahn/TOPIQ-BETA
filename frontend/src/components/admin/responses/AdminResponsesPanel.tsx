...
import type { DeleteDialog } from "./useAdminResponses";
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div><p className="text-xs font-semibold tracking-[.14em] text-primary">RESPONSE DATA</p><h2 className="mt-2 text-2xl font-semibold">응시 세션별 사용자 응답</h2><p className="mt-2 text-sm font-medium text-gray-500">총 {total.toLocaleString()}개의 응시 세션</p></div>
        <div className="flex flex-wrap gap-2">
          <select aria-label="세션 상태" value={status} onChange={(event) => onStatusChange(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><option value="">전체 상태</option><option value="submitted">제출 완료</option><option value="abandoned">폐기됨</option></select>
          ...
          {status === "abandoned" && <button onClick={() => onDeleteRequest({ mode: "abandoned" })} className="rounded-xl border border-red-300 bg-red-50 px-4 py-2.5 text-sm font-semibold text-red-700">폐기 세션 전체 삭제</button>}
