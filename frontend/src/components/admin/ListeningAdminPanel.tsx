import {
  ArrowLeft, AudioLines, CalendarDays, CheckCircle2, CircleAlert, Headphones,
  Image, LoaderCircle, Play, Plus, RefreshCw, Search, Settings2, Sparkles, Trash2, Upload,
} from "lucide-react";
  ...
  if (!selectedSet) {
    ...
    return <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div>
        <p className="text-xs font-semibold tracking-[.14em] text-primary">LISTENING ROUNDS</p>
        <h2 className="mt-2 text-2xl font-semibold">듣기 회차 관리</h2>
        <p className="mt-2 text-sm font-medium text-gray-500">SQL에 추가된 듣기 세트를 자동으로 감지합니다. 회차를 선택해 음원과 그림을 준비하세요.</p>
      </div><div className="flex gap-2 text-xs font-semibold"><span className="rounded-full bg-primary-50 px-3 py-2 text-primary">등록 회차 {linkedCount}</span><span className="rounded-full bg-gray-100 px-3 py-2 text-gray-600">새 세트 {pendingCount}</span></div></div>
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{sets.map((set) => {
        ...
        return <article key={`${set.setId}-${set.setVersion}`} data-testid="listening-set-card" className="rounded-2xl border border-gray-200 p-5">
          <div className="flex items-start justify-between gap-3"><span className={`rounded-full px-3 py-1.5 text-xs font-semibold ${linked?"bg-primary-100 text-primary-dark":"bg-gray-100 text-gray-700"}`}>{set.round!==null?`듣기 ${set.round}회`:"새 듣기 세트"}</span><span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${set.mockTestPublished?"bg-green-50 text-green-700":"bg-gray-100 text-gray-500"}`}>{set.mockTestPublished?"홈페이지 공개":linked?"비공개":"미등록"}</span></div>
          <h3 className="mt-4 text-lg font-semibold">{set.titleKo??`듣기 세트 · v${set.setVersion}`}</h3><p className="mt-1 truncate text-[11px] font-medium text-gray-400" title={set.setId}>{set.setId}</p>
          <div className="mt-4 grid grid-cols-3 gap-2 text-xs font-semibold"><p className="rounded-xl bg-gray-50 p-3"><Headphones className="mb-2 size-4 text-primary"/>문항 <b className="float-right">{set.itemCount}/50</b></p><p className="rounded-xl bg-gray-50 p-3"><AudioLines className="mb-2 size-4 text-primary"/>음원 <b className="float-right">{set.audioReady}/50</b></p><p className="rounded-xl bg-gray-50 p-3"><Image className="mb-2 size-4 text-primary"/>그림 <b className="float-right">{set.visualReady}/{set.visualRequired}</b></p></div>
          <p className="mt-3 flex items-center gap-2 text-xs font-semibold text-gray-400"><CalendarDays className="size-3.5"/>{new Date(set.createdAt).toLocaleDateString("ko-KR")}</p>
          {!linked && <div className="mt-3">{set.readyToRegister?<p className="flex items-center gap-2 text-xs font-semibold text-green-700"><CheckCircle2 className="size-4"/>비공개 회차 생성 준비 완료</p>:set.blockingReasons.map((reason)=><p key={reason} className="flex items-center gap-2 text-xs font-semibold text-gray-600"><CircleAlert className="size-3.5"/>{blockReasonLabels[reason]}</p>)}</div>}
          <div className="mt-4 flex flex-wrap gap-2"><button onClick={()=>void openSet(set)} className="focus-ring min-h-11 flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold">회차 열기</button>{!linked&&<button disabled={!set.readyToRegister||Boolean(busy)} onClick={()=>{if(window.confirm("이 세트를 새 비공개 듣기 회차로 등록할까요?"))void run(registerKey,()=>adminApi.registerListeningSet(token,set.setId,set.setVersion));}} className="focus-ring flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40">{busy===registerKey?<LoaderCircle className="size-4 animate-spin"/>:<Plus className="size-4"/>} 회차 생성</button>}</div>
        </article>;
      })}</div>{!sets.length&&<p className="py-16 text-center text-sm font-semibold text-gray-400">듣기 세트가 없습니다.</p>}
    </section>;
  }

  return <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7">
    <div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end"><div><button onClick={()=>{setSelectedSet(null);setItems([]);setAudioUrl("");}} className="focus-ring mb-4 flex min-h-11 items-center gap-2 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold"><ArrowLeft className="size-4"/>회차 목록</button><p className="text-xs font-semibold tracking-[.14em] text-primary">LISTENING BANK</p><h2 className="mt-2 text-2xl font-semibold">{selectedSet.round!==null?`듣기 ${selectedSet.round}회`:"새 듣기 세트"} · 문항 관리</h2><p className="mt-2 text-xs font-semibold text-gray-400">문항 {selectedSet.itemCount}/50 · 음원 {selectedSet.audioReady}/50 · 그림 {selectedSet.visualReady}/{selectedSet.visualRequired}</p></div>
      <div className="flex flex-wrap gap-2"><button disabled={Boolean(busy)} onClick={()=>void run("bulk-audio",()=>adminApi.generateSet(token,selectedSet.setId,selectedSet.setVersion,false,ttsStyle))} className="min-h-11 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold text-primary"><AudioLines className="mr-2 inline size-4"/>누락 음원 생성</button><button disabled={Boolean(busy)} onClick={()=>void run("bulk-visual",()=>adminApi.generateSetVisuals(token,selectedSet.setId,selectedSet.setVersion,false))} className="min-h-11 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white"><Sparkles className="mr-2 inline size-4"/>누락 그림 생성</button>{selectedSet.mockTestId&&<button disabled={!selectedSet.readyToPublish||Boolean(busy)} onClick={()=>void run("publish",()=>adminApi.publish(token,selectedSet.mockTestId!,!selectedSet.mockTestPublished))} className="min-h-11 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold disabled:opacity-40">{selectedSet.mockTestPublished?"비공개 전환":"시험 공개"}</button>}</div>
    </div>
    <div className="mt-6 grid gap-4 rounded-2xl bg-gray-50 p-4 lg:grid-cols-[.6fr_1.4fr_auto]"><label className="text-sm font-semibold">말하기 속도 <span className="text-primary">{ttsStyle.speakingRate.toFixed(2)}×</span><input type="range" min="0.75" max="1.25" step="0.05" value={ttsStyle.speakingRate} onChange={(event)=>setTtsStyle((current)=>({...current,speakingRate:Number(event.target.value)}))} className="mt-2 block w-full accent-primary"/></label><label className="text-sm font-semibold">음원 스타일 지시<input value={ttsStyle.stylePrompt} maxLength={300} onChange={(event)=>setTtsStyle((current)=>({...current,stylePrompt:event.target.value}))} className="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2.5" placeholder="차분한 시험 방송처럼 또렷하게"/></label><label className="flex min-h-11 items-center gap-2 self-end rounded-xl border border-gray-200 bg-white px-3"><Search className="size-4 text-gray-400"/><input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="번호·유형 검색" className="w-40 py-2.5 text-sm outline-none"/></label></div>
    ...
    {loading?<div className="grid min-h-48 place-items-center"><LoaderCircle className="size-6 animate-spin text-primary"/></div>:<div className="mt-6 space-y-4">{visibleItems.map((group)=><details key={`${group.setId}-${group.positions.join("-")}`} className="rounded-2xl border border-gray-200 p-4"><summary className="flex cursor-pointer list-none items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary text-sm font-semibold text-white">{positionLabel(group.positions)}</span><span className="min-w-0 flex-1"><b className="block truncate text-sm">{group.questionPrompts.join(" · ")}</b><span className="text-xs font-semibold text-gray-400">{group.itemType}{group.positions.length>1?" · 공통 음원":""}</span></span><span className={`rounded-lg px-2 py-1 text-xs font-semibold ${group.audioStatus==="ready"?"bg-green-50 text-green-700":"bg-orange-50 text-orange-700"}`}>{group.audioStatus==="ready"?"음원 준비":"음원 누락"}</span></summary>
      ...
      </div></details>)}{!visibleItems.length&&<p className="py-16 text-center text-sm font-semibold text-gray-400">표시할 문항이 없습니다.</p>}</div>}
  </section>;
}
