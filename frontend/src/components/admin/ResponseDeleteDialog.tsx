...
export function ResponseDeleteDialog({ dialog, selectedCount, confirmation, busy, onConfirmationChange, onCancel, onConfirm }: {
  ...
  const allResponses = dialog.mode === "all";
  return (
    <div className="fixed inset-0 z-50 grid place-items-center p-5">
      <div role="dialog" aria-modal="true" className="w-full max-w-md rounded-2xl bg-white p-7">
        ...
        <h2 className="mt-5 text-xl font-semibold">{allResponses ? "전체 응답 삭제" : `${selectedCount}개 세션 삭제`}</h2>
        ...
        {allResponses && <label className="mt-5 block text-sm font-semibold text-red-700">확인을 위해 “전체 응답 삭제”를 입력하세요.<input autoFocus value={confirmation} onChange={(event) => onConfirmationChange(event.target.value)} className="mt-2 w-full rounded-xl border border-red-200 px-4 py-3 text-gray-900" /></label>}
        <div className="mt-6 flex justify-end gap-2"><button onClick={onCancel} className="rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold">취소</button><button disabled={busy || (allResponses && confirmation !== "전체 응답 삭제")} onClick={onConfirm} className="rounded-xl bg-red-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">삭제</button></div>
      </div>
    </div>
  );
}
