    ...
    return (
      <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <p className="text-xs font-semibold tracking-[.14em] text-primary">READING ROUNDS</p>
            <h2 className="mt-2 text-2xl font-semibold">읽기 회차 관리</h2>
            <p className="mt-2 text-sm font-medium text-gray-500">회차를 먼저 선택한 뒤 해당 세트의 개별 문항을 확인할 수 있습니다.</p>
          </div>
          <div className="flex gap-2 text-xs font-semibold">
            <span className="rounded-full bg-primary-50 px-3 py-2 text-primary">공개 회차 {linkedCount}</span>
            <span className="rounded-full bg-gray-100 px-3 py-2 text-gray-600">새 세트 {pendingCount}</span>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {sets.map((set) => {
            ...
            return (
              <article key={`${set.setId}-${set.setVersion}`} data-testid="reading-set-card" className="rounded-2xl border border-gray-200 bg-white p-5">
                <div className="flex items-start justify-between gap-3">
                  <span className={`rounded-full px-3 py-1.5 text-xs font-semibold ${isLinked ? "bg-primary-100 text-primary-dark" : "bg-gray-100 text-gray-700"}`}>
                    {set.round !== null ? `읽기 ${set.round}회` : "새 읽기 세트"}
                  </span>
                  <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${set.mockTestPublished ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>
                    {set.mockTestPublished ? "홈페이지 공개" : isLinked ? "비공개" : "미등록"}
                  </span>
                </div>
                <h3 className="mt-4 text-lg font-semibold text-gray-900">{set.titleKo ?? `읽기 세트 · v${set.setVersion}`}</h3>
                <p className="mt-1 truncate text-[11px] font-medium text-gray-400" title={set.setId}>{set.setId}</p>
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs font-semibold">
                  <p className="rounded-xl p-3"><BookOpen className="mb-2 size-4 text-primary" />문항 <strong className="float-right text-gray-700">{set.itemCount}/50</strong></p>
                  <p className="rounded-xl p-3"><CalendarDays className="mb-2 size-4 text-gray-500" />{new Date(set.createdAt).toLocaleDateString("ko-KR")}</p>
                </div>
        ...
        {!sets.length && <p className="py-16 text-center text-sm font-semibold text-gray-400">읽기 세트가 없습니다.</p>}
      </section>
    );
  }
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <button type="button" onClick={() => { setSelectedSet(null); setItems([]); setSearch(""); }} className="focus-ring mb-4 flex min-h-11 items-center gap-2 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-semibold text-gray-700"><ArrowLeft className="size-4" /> 회차 목록</button>
          <p className="text-xs font-semibold tracking-[.14em] text-primary">READING BANK</p>
          <h2 className="mt-2 text-2xl font-semibold">{selectedSet.round !== null ? `읽기 ${selectedSet.round}회` : "새 읽기 세트"} · 개별 문항</h2>
          <p className="mt-2 flex items-center gap-2 text-xs font-semibold text-gray-400"><Globe2 className="size-3.5" />{selectedSet.itemCount}문항 · 세트 v{selectedSet.setVersion}</p>
        </div>
        <label className="flex min-h-11 items-center gap-2 rounded-xl border border-gray-200 px-3"><Search className="size-4 text-gray-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="번호·유형·본문 검색" className="w-56 py-2.5 text-sm font-medium outline-none" /></label>
      </div>

      {loadingItems ? <div className="grid min-h-48 place-items-center"><LoaderCircle className="size-6 animate-spin text-primary" /></div> : <div className="mt-6 space-y-3">
        ...
        {!visibleItems.length && <p className="py-16 text-center text-sm font-semibold text-gray-400">표시할 문항이 없습니다.</p>}
      </div>}
    </section>
  );
}
