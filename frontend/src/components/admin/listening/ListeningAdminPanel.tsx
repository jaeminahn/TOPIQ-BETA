...
export function ListeningAdminPanel({
  ...
  const [historyItem,setHistoryItem] = useState<{ setId:string; itemId:string; position:number }|null>(null);
  ...
  const startGroupGeneration = async () => {
    ...
    setItems((current) => current.map((group) => group.leaderItemId === leaderItemId
      ...
      : group));
    try {
      const result = await adminApi.generateGroup(token,selectedAudioGroup.setId,leaderItemId,Boolean(selectedAudioGroup.audioAssetId),style);
  ...
  const startBulkGeneration = async () => {
    ...
    setItems((current) => current.map((group) => candidateIds.has(group.leaderItemId)
      ...
      : group));
    try {
      const result = await adminApi.generateSet(token,selectedSet.setId,false,style);
  ...
  return <section className={`rounded-2xl border border-gray-200 bg-white p-5 sm:p-7 ${dock ? "pb-[25rem] lg:pb-48" : ""}`}>
    ...
    {loading?<AdminRoundLoading />:<div className="mt-6 space-y-4">{visibleItems.map((group)=>{const generating=isAudioGenerating(group);return <details key={`${group.setId}-${group.positions.join("-")}`} className="rounded-2xl border border-gray-200 p-4 open:border-primary-100 open:bg-primary-50"><summary className="flex cursor-pointer list-none items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary text-sm font-semibold text-white">{positionLabel(group.positions)}</span><span className="min-w-0 flex-1"><b className="block truncate text-sm">{group.questionPrompts.join(" · ")}</b><span className="text-xs font-semibold text-gray-400">{group.itemType}{group.positions.length>1?" · 공통 음원":""}</span></span><span className={`rounded-lg px-2 py-1 text-xs font-semibold ${generating?"bg-primary-50 text-primary":group.audioStatus==="ready"?"bg-green-50 text-green-700":"bg-orange-50 text-orange-700"}`}>{generating?"음원 생성 중":group.audioStatus==="ready"?"완성형 음원 준비":group.audioStatus==="legacy"?"구형 음원 · 재생성 필요":"음원 누락"}</span></summary>
      <div className="mt-5 border-t border-gray-200 pt-5">
        <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
          <div className="max-h-36 overflow-auto text-sm leading-6">{group.dialogueTurns.map((turn,index)=><p key={index}><b>{turn.speaker}</b> {turn.text}</p>)}{group.lastError&&<p className="mt-2 text-xs font-semibold text-red-600">{group.lastError}</p>}</div>
          <div className="flex flex-wrap content-start gap-2">
            {group.audioAssetId&&<button disabled={generating||Boolean(busy)} onClick={()=>openGroupDock(group,true)} className="flex min-h-10 items-center gap-1 rounded-lg border border-gray-200 px-3 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-40"><Play className="size-3"/>재생</button>}
            <button disabled={Boolean(busy)} onClick={()=>openGroupDock(group,false)} className="min-h-10 rounded-lg border border-gray-200 px-3 text-xs font-semibold text-primary disabled:opacity-40">{generating?"생성 상태 보기":group.audioAssetId?"음원 재생성":"음원 생성"}</button>
            {group.audioAssetId&&<button disabled={generating||Boolean(busy)} onClick={()=>{if(window.confirm(`${positionLabel(group.positions)}번 음원을 Supabase에서도 삭제할까요?`)){if(dock?.kind==="group"&&dock.leaderItemId===group.leaderItemId)closeDock();void run(`delete-audio-${group.leaderItemId}`,()=>adminApi.deleteGroupAudio(token,group.setId,group.leaderItemId,group.audioAssetId!));}}} className="min-h-10 rounded-lg border border-red-200 px-3 text-xs font-semibold text-red-600 disabled:opacity-40"><Trash2 className="mr-1 inline size-3"/>삭제</button>}
          </div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2">{group.targets.map((target)=><div key={target.itemId} className="flex min-h-11 items-center rounded-xl border border-gray-200 bg-white px-2"><button type="button" onClick={()=>setEditing({group,target})} className="min-w-0 flex-1 px-2 text-left text-xs font-semibold"><span className="truncate">{target.position}번 · {target.questionPrompt}</span><span className="ml-2 inline-flex items-center gap-1 text-primary"><Pencil className="size-3"/>수정</span></button><button type="button" aria-label={`${target.position}번 버전 이력`} onClick={()=>setHistoryItem({setId:group.setId,itemId:target.itemId,position:target.position})} className="flex min-h-9 items-center gap-1 rounded-lg border border-gray-200 px-2 text-xs font-semibold text-gray-600"><History className="size-3"/>이력</button></div>)}</div>
    ...
    {editing&&<AdminQuestionEditorDialog token={token} section="listening" setId={editing.group.setId} question={editing.target} groupQuestions={editing.group.targets} onClose={()=>setEditing(null)} onSaved={()=>{closeDock();setEditing(null);void Promise.all([loadItems(selectedSet),onSetsChanged()]);}}/>}
