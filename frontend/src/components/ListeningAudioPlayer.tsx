...
export function ListeningAudioPlayer({
  ...
  return (
    <section className="mb-3 rounded-xl border border-gray-200 bg-white px-3 py-2.5 sm:px-4">
      <div className="flex min-w-0 flex-wrap items-center gap-2.5">
        <span className="grid size-9 shrink-0 place-items-center rounded-lg bg-gray-100 text-gray-600"><Headphones className="size-5" /></span>
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-semibold uppercase tracking-[.12em] text-gray-700">{t("listening")}</p>
          <p className="mt-0.5 flex items-center gap-1.5 text-sm font-semibold text-gray-700">
        ...
        {mode === "timed" ? <div className="ml-auto flex items-center gap-2"><span className="rounded-lg bg-gray-100 px-2.5 py-1.5 text-xs font-semibold text-gray-600">{Math.min(displayPlayIndex, repeatCount)} / {repeatCount}</span>{(status === "blocked" || status === "error") && <button onClick={() => void (src && audioRef.current ? audioRef.current.play().catch(() => undefined) : start())} className="focus-ring flex min-h-11 items-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-dark"><Play className="size-4" /> {t("audioPlay")}</button>}</div> : <button onClick={() => void start()} disabled={status === "loading"} className="focus-ring ml-auto flex min-h-11 items-center gap-2 rounded-xl border border-gray-200 bg-gray-100 px-4 py-2 text-sm font-semibold text-gray-700 disabled:opacity-50 hover:bg-gray-200">{src ? <RotateCcw className="size-4" /> : <Play className="size-4" />} {src ? t("audioReplay") : t("audioPlay")}</button>}
      </div>
      {message && <p role="status" className="mt-2 text-sm font-semibold text-orange-500">{message}</p>}
