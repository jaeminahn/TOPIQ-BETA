...
import { localizedExamDescription, localizedExamTitle } from "../../examLocalization";
                ...
                <div className="grid gap-3 lg:grid-cols-2">{group.exams.map((exam) => {
                  ...
                  return (
                    <article key={exam.id} className="flex min-w-0 flex-col rounded-2xl border border-gray-200 bg-white p-4">
                      ...
                      <h4 className="mt-3 text-lg font-semibold text-gray-900">{localizedExamTitle(exam.titleKo, locale, { slug: exam.slug, section: exam.section })}</h4>
                      <p className="mt-1 line-clamp-2 min-h-10 text-sm leading-5 text-gray-600">{localizedExamDescription(exam.descriptionKo, locale, exam.section, exam.questionCount)}</p>
