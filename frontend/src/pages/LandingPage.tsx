import { ArrowRight, BookOpen, CheckCircle2, Clock3, Headphones, RotateCcw } from "lucide-react";
          ...
          <div className="mx-auto hidden w-full max-w-md lg:block">
            <div className="rounded-2xl border border-gray-200 bg-white p-5">
              <div className="flex items-center justify-between">
                <span className="rounded-full bg-primary-50 px-2.5 py-1 text-[11px] font-semibold text-primary">TOPIK II · READING</span>
              ...
              {examRounds.map((group, groupIndex) => {
                ...
                return <section key={group.key} data-testid="exam-round" data-round={group.round ?? "other"} className={`rounded-2xl border p-3 ${palette.shell}`}>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    <h3 className={`rounded-lg px-3 py-1.5 text-sm font-semibold ${palette.badge}`}>{roundLabel}</h3>
                    <span className="text-xs font-medium text-gray-400">TOPIK II</span>
                  </div>
                      ...
                      return (
                        <article key={exam.id} className="flex min-w-0 flex-col rounded-2xl border border-gray-200 bg-white p-4">
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <span className="flex items-center gap-1.5 rounded-full bg-gray-100 px-2.5 py-1 text-[11px] font-medium text-gray-500">{exam.section.toUpperCase()} · 50 × 2 PTS</span>
                          </div>
                          <h4 className="mt-3 text-lg font-semibold text-gray-900">{locale === "id" ? exam.titleId : exam.titleKo}</h4>
      ...
      <footer className="bg-gray-100 py-8 pb-16">
        <div className="mx-auto max-w-5xl px-4 sm:px-8">
          <div className="flex flex-col items-center justify-between gap-2 border-t border-gray-200 pt-5 text-xs text-gray-500 sm:flex-row">
            <span className="flex items-center gap-2 font-medium text-gray-700"><CheckCircle2 className="size-4 text-primary" /> UNIGATE TOPIK LAB</span>
