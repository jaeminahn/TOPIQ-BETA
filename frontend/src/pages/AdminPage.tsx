...
import type {
  AdminListeningGroup, AdminListeningMockTest, AdminListeningSet, AdminReadingSet,
...
export function AdminPage() {
  ...
  const [detectedListeningSets, setDetectedListeningSets] = useState<AdminListeningSet[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [dashboard, jobData, listeningSetData, readingSetData, responseData] = await Promise.all([
        ...
        adminApi.jobs(token), adminApi.listeningSets(token), adminApi.readingSets(token),
      ...
      setSummary(dashboard.summary); setJobs(jobData.jobs);
      setDetectedListeningSets(listeningSetData.sets); setReadingSets(readingSetData.sets);
  ...
  const loadDetectedListeningSets = useCallback(async () => {
    if (!token) return;
    try {
      const result = await adminApi.listeningSets(token);
      setDetectedListeningSets(result.sets);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "듣기 회차를 불러오지 못했습니다."); }
  }, [token]);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="mx-auto max-w-[1500px] px-5 py-7 sm:px-8">
        ...
        <div className={tab === "listening" ? "hidden" : ""}>
        {tab === "overview" && summary && <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white"><div className="border-b border-gray-100 p-6 sm:p-8"><p className="text-xs font-semibold tracking-[.14em] text-primary">DASHBOARD</p><h2 className="mt-2 text-2xl font-semibold">서비스 요약</h2></div><div className="grid divide-y divide-gray-100 lg:grid-cols-3 lg:divide-x lg:divide-y-0"><div className="p-6 sm:p-8"><h3 className="font-semibold">문제은행</h3><div className="mt-3 grid grid-cols-2"><Metric label="전체 문항" value={summary.totalItems} icon={Database} /><Metric label="전체 세트" value={summary.setCount} icon={LayoutDashboard} /><Metric label="읽기 문항" value={summary.readingVersions} icon={BookOpen} /><Metric label="듣기 문항" value={summary.listeningVersions} icon={Headphones} /></div></div><div className="p-6 sm:p-8"><h3 className="font-semibold">듣기 자산</h3><div className="mt-3 grid grid-cols-2"><Metric label="준비 음원" value={summary.audioReady} icon={AudioLines} /><Metric label="누락 음원" value={summary.audioMissing} icon={Headphones} /><Metric label="이미지" value={summary.visualReady} icon={Image} /><Metric label="진행 작업" value={summary.jobsQueued + summary.jobsProcessing} icon={Activity} /></div>{failedJobs.length > 0 && <details className="mt-4 rounded-xl bg-red-50 p-3"><summary className="cursor-pointer text-xs font-semibold text-red-700">최근 실패 {failedJobs.length}건</summary><div className="mt-3 space-y-2">{failedJobs.map((job) => <p key={job.jobId} className="break-all text-xs text-red-700">{job.errorMessage}</p>)}</div></details>}</div><div className="p-6 sm:p-8"><h3 className="font-semibold">사용자 응답</h3><div className="mt-3 grid grid-cols-2"><Metric label="전체 응답" value={summary.responseCount} icon={MessageSquareText} /><Metric label="실제 응답" value={summary.answeredResponseCount} icon={CheckCircle2} /><Metric label="미응답" value={summary.unansweredResponseCount} icon={XCircle} /><Metric label="오늘 세션" value={summary.sessionsToday} icon={Users} /></div></div></div></section>}

        {tab === "listening" && <><section className="grid gap-5 xl:grid-cols-2">{mockTests.map((test, index) => { const ready = test.audioReady === 50 && test.visualReady >= test.visualRequired; return <article key={test.mockTestId} className="rounded-2xl border border-gray-200 bg-white p-6"><div className="flex items-start justify-between gap-4"><div><span className="rounded-full bg-primary-50 px-3 py-1.5 text-xs font-semibold text-primary">듣기 {index + 1}회</span><h2 className="mt-3 text-xl font-semibold">{test.titleKo}</h2></div><span className={`rounded-full px-3 py-1.5 text-xs font-semibold ${test.published ? "bg-green-50 text-green-700" : "bg-gray-100 text-gray-500"}`}>{test.published ? "공개" : "비공개"}</span></div><div className="mt-5 grid grid-cols-2 gap-3 text-sm"><p className="rounded-xl bg-gray-50 p-3 font-semibold">음원 <span className="float-right text-gray-700">{test.audioReady}/50</span></p><p className="rounded-xl bg-gray-50 p-3 font-semibold">이미지 <span className="float-right text-gray-700">{test.visualReady}/{test.visualRequired}</span></p></div><div className="mt-4 flex flex-wrap gap-2"><button disabled={Boolean(busy)} onClick={() => void action(`set-${test.setId}`, () => adminApi.generateSet(token, test.setId, test.setVersion, false, ttsStyle))} className="rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">누락 음원 일괄 생성</button><button disabled={!ready || Boolean(busy)} onClick={() => void action(`publish-${test.mockTestId}`, () => adminApi.publish(token, test.mockTestId, !test.published))} className="rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold disabled:opacity-40">{test.published ? "비공개 전환" : "시험 공개"}</button></div></article>; })}</section><section className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 sm:p-7"><div className="flex items-center gap-2"><Settings2 className="size-5 text-primary" /><h2 className="text-lg font-semibold">새 음원 스타일</h2></div><p className="mt-2 text-sm font-medium text-gray-500">이후 생성·재생성하는 공통 음원 그룹에 적용됩니다.</p><div className="mt-5 grid gap-5 lg:grid-cols-[minmax(280px,.7fr)_1.3fr]"><label className="text-sm font-semibold text-gray-700">말하기 속도 <span className="ml-2 text-primary">{ttsStyle.speakingRate.toFixed(2)}×</span><input type="range" min="0.75" max="1.25" step="0.05" value={ttsStyle.speakingRate} onChange={(event) => setTtsStyle((current) => ({ ...current, speakingRate: Number(event.target.value) }))} className="mt-3 w-full accent-primary" /></label><label className="text-sm font-semibold text-gray-700">추가 스타일 지시<input maxLength={300} value={ttsStyle.stylePrompt} onChange={(event) => setTtsStyle((current) => ({ ...current, stylePrompt: event.target.value }))} placeholder="차분한 시험 방송처럼 또렷하게 읽어 주세요." className="mt-2 w-full rounded-xl border border-gray-200 px-4 py-3 font-medium" /></label></div></section><section className="mt-6 rounded-2xl border border-gray-200 bg-white p-5 sm:p-7"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-semibold tracking-[.14em] text-primary">LISTENING BANK</p><h2 className="mt-2 text-2xl font-semibold">듣기 문항·공통 음원</h2></div><div className="flex gap-2"><select value={setId} onChange={(event) => setSetId(event.target.value)} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><option value="">전체 세트</option>{listeningSets.map((set, index) => <option key={set.setId} value={set.setId}>듣기 {index + 1}회</option>)}</select><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><option value="">전체 상태</option><option value="ready">음원 준비</option><option value="missing">음원 누락</option><option value="failed">생성 실패</option></select></div></div><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-y border-gray-200 bg-gray-50 text-xs font-semibold text-gray-500"><tr><th className="p-3">번호</th><th className="p-3">유형</th><th className="p-3">공통 대화·질문</th><th className="p-3">음원</th><th className="p-3">그림</th><th className="p-3">작업</th></tr></thead><tbody>{items.map((group) => <tr key={`${group.setId}-${group.positions.join("-")}`} className="border-b border-gray-100 align-top"><td className="p-3"><span className="rounded-xl bg-primary-50 px-3 py-2 font-semibold text-primary">{positionLabel(group.positions)}</span>{group.positions.length > 1 && <span className="mt-2 block text-[10px] font-semibold text-gray-400">음원 1개 공유</span>}</td><td className="p-3"><span className="rounded-lg bg-gray-100 px-2 py-1 text-xs font-semibold">{group.itemType}</span></td><td className="max-w-lg p-3"><div className="max-h-28 space-y-1 overflow-auto text-xs leading-5">{group.dialogueTurns.map((turn, index) => <p key={index}><b className="text-gray-700">{turn.speaker}</b> {turn.text}</p>)}</div><div className="mt-2 text-[11px] font-semibold text-gray-400">{group.questionPrompts.join(" · ")}</div>{group.lastError && <details className="mt-2 rounded-lg bg-red-50 p-2 text-xs text-red-700"><summary className="cursor-pointer font-semibold">마지막 생성 오류</summary><p className="mt-2 break-all">{group.lastError}</p></details>}</td><td className="p-3">{group.audioAssetId ? <div className="space-y-2"><button onClick={() => void adminApi.audioUrl(token, group.audioAssetId!).then((data) => setAudioUrl(data.audioUrl))} className="flex items-center gap-1 rounded-lg bg-primary-50 px-2.5 py-2 text-xs font-semibold text-primary"><Play className="size-3" /> 재생</button><span className="block text-[11px] font-semibold text-gray-400">{group.ttsStyle?.speakingRate?.toFixed(2) ?? "1.00"}×</span></div> : <span className="font-semibold text-orange-500">{group.audioStatus === "partial" ? "일부 누락" : "누락"}</span>}</td><td className="p-3">{group.targets.some((target) => target.visualOptionCount > 0) ? group.targets.map((target) => target.visualOptionCount > 0 && <div key={target.itemId}><p className="mb-2 text-xs font-semibold">{target.position}번 {target.visualReadyCount}/{target.visualOptionCount}</p><div className="flex gap-1">{Array.from({ length: target.visualOptionCount }, (_, index) => <label key={index} className="cursor-pointer rounded-md border border-gray-200 px-2 py-1 text-xs font-semibold hover:bg-gray-100">{index + 1}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void action(`upload-${target.itemId}-${index}`, () => adminApi.uploadVisual(token, target.itemId, target.itemVersion, index + 1, file)); }} /></label>)}</div></div>) : "—"}</td><td className="p-3"><div className="flex flex-wrap gap-2"><button disabled={Boolean(busy)} onClick={() => void action(`group-${group.leaderItemId}`, () => adminApi.generateGroup(token, group.setId, group.setVersion, group.leaderItemId, Boolean(group.audioAssetId), ttsStyle))} className="rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold text-primary disabled:opacity-50">{group.audioAssetId ? "재생성" : "생성"}</button>{group.audioAssetId && <button disabled={Boolean(busy)} onClick={() => { if (window.confirm(`${positionLabel(group.positions)}번 공통 음원을 삭제할까요?`)) void action(`delete-${group.leaderItemId}`, () => adminApi.deleteGroupAudio(token, group.setId, group.setVersion, group.leaderItemId, group.audioAssetId!)); }} className="flex items-center gap-1 rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600"><Trash2 className="size-3" /> 삭제</button>}</div></td></tr>)}</tbody></table></div></section></>}

        </div>
        {tab === "listening" && <ListeningAdminPanel token={token} sets={detectedListeningSets} onError={setError} onSetsChanged={loadDetectedListeningSets} />}
        ...
        {tab === "responses" && <section className="rounded-2xl border border-gray-200 bg-white p-5 sm:p-7"><div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-semibold tracking-[.14em] text-primary">RESPONSE DATA</p><h2 className="mt-2 text-2xl font-semibold">응시 세션별 사용자 응답</h2><p className="mt-2 text-sm font-medium text-gray-500">총 {responseTotal.toLocaleString()}개의 제출 세션</p></div><div className="flex flex-wrap gap-2"><select value={responseSection} onChange={(event) => { setResponseSection(event.target.value); setResponsePage(1); }} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><option value="">전체 영역</option><option value="reading">읽기</option><option value="listening">듣기</option></select><select value={responseCorrectness} onChange={(event) => { setResponseCorrectness(event.target.value); setResponsePage(1); }} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm font-semibold"><option value="">전체 결과</option><option value="correct">정답 포함</option><option value="incorrect">오답 포함</option><option value="unanswered">미응답 포함</option></select><button disabled={!selectedSessions.size} onClick={() => setDeleteDialog({ mode: "selected" })} className="rounded-xl border border-red-200 px-4 py-2.5 text-sm font-semibold text-red-600 disabled:opacity-40">선택 삭제 ({selectedSessions.size})</button><button onClick={() => setDeleteDialog({ mode: "all" })} className="rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white">전체 삭제</button></div></div><div className="mt-6 overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-y border-gray-200 bg-gray-50 text-xs font-semibold text-gray-500"><tr><th className="p-3"><input type="checkbox" checked={allPageSelected} onChange={() => setSelectedSessions((current) => { const next = new Set(current); for (const session of responseSessions) allPageSelected ? next.delete(session.sessionId) : next.add(session.sessionId); return next; })} aria-label="현재 페이지 전체 선택" /></th><th className="p-3">제출 시각</th><th className="p-3">시험</th><th className="p-3">점수</th><th className="p-3">실제/미응답</th><th className="p-3">정답/오답</th><th className="p-3">별점</th><th className="p-3">상세</th></tr></thead><tbody>{responseSessions.map((session) => <><tr key={session.sessionId} className="border-b border-gray-100"><td className="p-3"><input type="checkbox" checked={selectedSessions.has(session.sessionId)} onChange={() => setSelectedSessions((current) => { const next = new Set(current); next.has(session.sessionId) ? next.delete(session.sessionId) : next.add(session.sessionId); return next; })} aria-label={`${session.sessionId} 선택`} /></td><td className="p-3 text-xs text-gray-500">{new Date(session.submittedAt).toLocaleString("ko-KR")}</td><td className="p-3"><b className="block text-xs">{session.mockTestTitle}</b><span className="text-xs text-gray-400">{session.mode === "timed" ? "실전" : "연습"}</span></td><td className="p-3 font-semibold text-primary">{session.score}/{session.maxScore}</td><td className="p-3"><b>{session.answeredCount}</b> / <span className="text-orange-500">{session.unansweredCount}</span></td><td className="p-3"><span className="text-green-700">{session.correctCount}</span> / <span className="text-red-600">{session.incorrectCount}</span></td><td className="p-3 font-semibold">{session.rating ? `${session.rating}점` : "—"}</td><td className="p-3"><button onClick={() => void toggleDetails(session.sessionId)} className="flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold">보기 <ChevronDown className={`size-3 transition ${expandedSession === session.sessionId ? "rotate-180" : ""}`} /></button></td></tr>{expandedSession === session.sessionId && <tr key={`${session.sessionId}-detail`}><td colSpan={8} className="bg-gray-50 p-4"><div className="grid grid-cols-2 gap-2 sm:grid-cols-5 lg:grid-cols-10">{responseDetails[session.sessionId]?.map((response) => <div key={response.observationId} className={`rounded-xl border p-3 text-xs ${response.isCorrect ? "border-green-200 bg-green-50" : response.selectedOption ? "border-red-200 bg-red-50" : "border-orange-200 bg-orange-50"}`}><b className="text-sm">{response.itemOrder}번</b><p className="mt-1 truncate text-gray-500">{response.itemType}</p><p className="mt-2 font-semibold">{response.selectedOption ?? "미응답"} → {response.correctAnswer}</p><p className="mt-1 text-[10px] text-gray-400">{formatDuration(response.responseTimeMs)}{response.answerChanged ? " · 변경" : ""}</p></div>) ?? <p className="col-span-full py-6 text-center text-gray-400">불러오는 중...</p>}</div></td></tr>}</>)}</tbody></table></div>{!responseSessions.length && <p className="py-16 text-center text-sm font-semibold text-gray-400">응답 세션이 없습니다.</p>}<div className="mt-5 flex items-center justify-end gap-3"><button disabled={responsePage <= 1} onClick={() => setResponsePage((page) => page - 1)} className="rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold disabled:opacity-40">이전</button><span className="text-xs font-semibold text-gray-500">{responsePage} / {pageCount}</span><button disabled={responsePage >= pageCount} onClick={() => setResponsePage((page) => page + 1)} className="rounded-lg border border-gray-200 px-3 py-2 text-xs font-semibold disabled:opacity-40">다음</button></div></section>}

        {audioUrl && <div className="fixed bottom-5 left-1/2 z-30 flex w-[min(92vw,600px)] -translate-x-1/2 items-center gap-3 rounded-2xl bg-gray-900 p-3 text-white"><audio src={audioUrl} controls autoPlay className="w-full" /><button onClick={() => setAudioUrl("")} className="px-2 font-semibold">×</button></div>}
      </main>

      {deleteDialog && <div className="fixed inset-0 z-50 grid place-items-center p-5"><div role="dialog" aria-modal="true" className="w-full max-w-md rounded-2xl bg-white p-7"><div className="grid size-11 place-items-center rounded-2xl bg-red-50 text-red-600"><Trash2 className="size-5" /></div><h2 className="mt-5 text-xl font-semibold">{deleteDialog.mode === "all" ? "전체 응답 삭제" : `${selectedSessions.size}개 세션 삭제`}</h2><p className="mt-3 text-sm font-medium leading-6 text-gray-600">응답, 답안 변경 이벤트, 별점과 재생 기록이 함께 삭제되며 복구할 수 없습니다. 이메일 수신 동의는 유지됩니다.</p>{deleteDialog.mode === "all" && <label className="mt-5 block text-sm font-semibold text-red-700">확인을 위해 “전체 응답 삭제”를 입력하세요.<input autoFocus value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} className="mt-2 w-full rounded-xl border border-red-200 px-4 py-3 text-gray-900" /></label>}<div className="mt-6 flex justify-end gap-2"><button onClick={() => { setDeleteDialog(null); setDeleteConfirmation(""); }} className="rounded-xl border border-gray-200 px-4 py-3 text-sm font-semibold">취소</button><button disabled={Boolean(busy) || (deleteDialog.mode === "all" && deleteConfirmation !== "전체 응답 삭제")} onClick={() => void confirmDeletion()} className="rounded-xl bg-red-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-40">삭제</button></div></div></div>}
    </div>
  );
}
